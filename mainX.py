...
class ColorGenerator(QObject):
    def __init__(self, color, resolution=QSize(640, 480), parent=None):
        super().__init__(parent)
...
class RandomNoiseGenerator(QObject):
    def __init__(self, synch, resolution=QSize(640, 480), parent=None):
        super().__init__(parent)
        self._frame = np.random.randint(0, 255, (resolution.height(), resolution.width(), 3), dtype=np.uint8)
        synch.synch_SIGNAL.connect(self.captureFrame)

    def captureFrame(self):
        self._frame = np.random.randint(0, 255, (self._frame.shape[0], self._frame.shape[1], 3), dtype=np.uint8)
...
class MainWindowX(QMainWindow):
    def __init__(self, something=None, parent=None):
        ...
        self.input1 = ColorGenerator((255, 0, 0))
        self.input2 = RandomNoiseGenerator(self.synch)
        self.mixBus = MixBus015T(self.input1, self.input2)
