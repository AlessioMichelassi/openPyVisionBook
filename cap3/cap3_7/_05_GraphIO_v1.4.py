...
class ImageProcessor:
    ...
    @staticmethod
    def apply_linear_contrast(img):
        min_val = np.min(img)
        max_val = np.max(img)
        return ((img - min_val) / (max_val - min_val) * 255).astype(np.uint8)

    @staticmethod
    def apply_local_contrast(img):
...
class GraphWidget(QWidget):
    def __init__(self):
        ...
        self.clahe_button = QPushButton('CLAHE', self)
        self.linear_contrast_button = QPushButton('Linear Contrast', self)
        self.local_contrast_button = QPushButton('Local Contrast', self)
    ...
    def init_ui(self):
        ...
        button_layout.addWidget(self.clahe_button)
        button_layout.addWidget(self.linear_contrast_button)
        button_layout.addWidget(self.local_contrast_button)
    ...
    def init_connections(self):
        ...
        self.linear_contrast_button.clicked.connect(self.apply_linear_contrast)
        self.local_contrast_button.clicked.connect(self.apply_local_contrast)
    ...
    def apply_clahe(self):
        if self.orig_img is not None:
            img = ImageProcessor.apply_clahe(self.orig_img)
            self.display_image(img, self.image_label)
            hist_img = ImageProcessor.compute_histogram(img)
            self.hist_label.setPixmap(QPixmap.fromImage(hist_img))
    ...
    def apply_linear_contrast(self):
        if self.orig_img is not None:
            img = ImageProcessor.apply_linear_contrast(self.orig_img)
            self.display_image(img, self.image_label)
            hist_img = ImageProcessor.compute_histogram(img)
            self.hist_label.setPixmap(QPixmap.fromImage(hist_img))

    def apply_local_contrast(self):
        if self.orig_img is not None:
            img = ImageProcessor.apply_local_contrast(self.orig_img)
            self.display_image(img, self.image_label)
            hist_img = ImageProcessor.compute_histogram(img)
            self.hist_label.setPixmap(QPixmap.fromImage(hist_img))
