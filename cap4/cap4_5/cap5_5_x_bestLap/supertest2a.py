...
def test():
    ...
    timeStart = time.time()
    stinger_frames, alpha_frames, inv_alphas = profile_loadStingerFrames(path, cv2.split, cv2.merge, invertCv2, "original")
    print(f"Time to load stinger frames (cv2.split, cv2.merge): {time.time() - timeStart:.4f} seconds")

    timeStart = time.time()
    stinger_frames2, alpha_frames2, inv_alphas2 = profile_loadStingerFrames(path, splitNumpyIndex, cv2.merge, invertCv2, "fast")
    print(f"Time to load stinger frames2 (numpy index, cv2.merge): {time.time() - timeStart:.4f} seconds")

    timeStart = time.time()
    stinger_frames3, alpha_frames3, inv_alphas3 = profile_loadStingerFrames(path, splitNumpyIndex, mergeListArray, invertCv2, "version3")
