...
def test():
    ...
    # Profiling delle funzioni di stinger con più iterazioni
    profiler = cProfile.Profile()
    profiler.enable()
    for _ in range(10000):
        stingerFunction(input1, input2, stinger_frames, alpha_frames, inv_alphas)
    profiler.disable()
    ...
    profiler.enable()
    for _ in range(10000):
        stingerFunction(input1, input2, stinger_frames2, alpha_frames2, inv_alphas2)
    profiler.disable()
    ...
    profiler.enable()
    for _ in range(10000):
        stingerFunction(input1, input2, stinger_frames3, alpha_frames3, inv_alphas3)
    profiler.disable()
...
if __name__ == "__main__":
    test()
    # Visualizzazione grafica dei risultati del profiling
    import subprocess
    subprocess.run(["snakeviz", "stinger_stats_original"])
    subprocess.run(["snakeviz", "stinger_stats_fast"])
    subprocess.run(["snakeviz", "stinger_stats_version3"])
